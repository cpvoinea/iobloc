...
namespace iobloc.Native
{
    ...
    internal abstract class ConsoleStream : Stream
    {
        ...
        protected void ValidateRead(byte[] buffer, int offset, int count)
        {
            ...
            if (offset < 0 || count < 0)
                throw new System.ArgumentOutOfRangeException(offset < 0 ? nameof(offset) : nameof(count), SR.ArgumentOutOfRange_NeedNonNegNum);
            if (buffer.Length - offset < count)
                throw new System.ArgumentException(SR.Argument_InvalidOffLen);
