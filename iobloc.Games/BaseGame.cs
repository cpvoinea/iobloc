...
namespace iobloc
{
    ...
    public abstract class BaseGame : IBaseGame<PaneCell>
    {
        ...
        public Dictionary<string, Pane<PaneCell>> Panes { get; private set; }
        // Summary:
        ...
        public IGame<PaneCell> Next { get; protected set; }
        // Summary:
        //      Main pane inside border rectangle
        protected Pane<PaneCell> Main { get; set; }
        // Summary:
        ...
        protected virtual void InitializeUI()
        {
            ...
            Panes = new Dictionary<string, Pane<PaneCell>>();

            // don't use main for these games
            if (Type != GameType.Table)
            {
                Main = new Pane<PaneCell>(1, 1, Height, Width);
            ...
            if (!new GameType[] { GameType.Fireworks, GameType.RainingBlood, GameType.Paint }.Contains(Type))
                Panes.Add(Pnl.Level, new Pane<PaneCell>(Border.Height - 1, (Border.Width + 1) / 2 - 2, Border.Height - 1, (Border.Width + 1) / 2));
            if (Serializer.Highscores.ContainsKey(ID)) // don't add score pane if game doesn't keep score
            {
                if (Border.Width > 8) // don't add highscore pane if there's no room
                    Panes.Add(Pnl.Highscore, new Pane<PaneCell>(0, 1, 0, 3));
                Panes.Add(Pnl.Score, new Pane<PaneCell>(0, Border.Width - 4, 0, Border.Width - 2));
            }
        }
